...
const tabUnpostButton = document.querySelector(".tab-unpost-button");
...
tabUnpostButton.addEventListener("click", () => {
    unpostUserTabBy_id(getTab_id()).then(e => {
        if (e.error) {
...
async function unpostUserTabBy_id(tab_id) {
    const response = await fetch("/unpostUserTabBy_id/" + tab_id + "/" + getToken(), {
        method: 'PATCH',
        mode: 'cors',
        headers: { 'Content-Type': 'application/json' }
    }).then(res => res.json());

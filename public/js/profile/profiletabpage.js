...
getTabByID(getTab_id()).then(userTab => {
    getUserByTabOwner_id(userTab.owner).then(res => {
        if (res.error) {
            alert(res.error);
        } else {
            tabOwnerTitle.innerHTML = "Owner: " + res.name;
            //tabOwnerTitle.href = "/publicprofile/" + getUser_idByTabUsername(userTab.owner) + "/" + getToken();
        }
    })
...
async function getUser_idByTabUsername(username) {
    const response = await fetch("/getuser_idbytagusername/" + username + "/" + getToken(), {
        method: 'GET',
        mode: 'cors',
        headers: { 'Content-Type': 'application/json' }
    }).then(res => res.json());

...
async function getUserTabsUnposted() {
    const response = await fetch("/getusertabsunposted/" + getToken(), {
        method: 'GET',
        mode: 'cors',
...
async function createUserTabs(userTabs) {
    for (let i = 0; i < userTabs.length; i++) {
        ...
        userTabName.href = "/tabedit/" + userTabs[i]._id + "/" + getToken();
...
getUserTabsUnposted().then(userTabs => {
